...
module std_conv3x3_layer #(
    ...
    input  wire [COUT*CIN*9*DW-1:0]          weight_flat,  // w[cout][cin][ky*3+kx]
...
function [DW-1:0] get_w;
    input integer coutc, cinc, kk;
    begin
        get_w = weight_flat[((coutc*CIN+cinc)*9+kk)*DW +: DW];
    end
endfunction
...
wire signed [7:0] cur_iy = (oy*STRIDE) + (cur_k/3) - 1;
wire signed [7:0] cur_ix = (ox*STRIDE) + (cur_k%3) - 1;

assign eng_x = get_act(cur_cin, cur_iy, cur_ix);
assign eng_w = get_w(cout, cur_cin, cur_k);
